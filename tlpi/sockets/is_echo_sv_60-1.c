...
#define CSEM_PATH_TMP "/tlpiexer-60-01"
#define MAX_CHILDREN 4

static sem_t *csem = NULL;
static char csem_path[PATH_MAX];
...
main(int argc, char *argv[])
{
    ...
    // close and unlink semaphore when done
    if (signal(SIGINT, term_handle) == SIG_ERR)
        errExit("signal:SIGINT");
    if (signal(SIGTERM, term_handle) == SIG_ERR)
        errExit("signal:SIGTERM");

    // init limit of children
    snprintf(csem_path, sizeof(csem_path), "%s-%i", CSEM_PATH_TMP, getpid());
    csem = sem_open(csem_path, O_CREAT | O_EXCL, 0, MAX_CHILDREN);
    ...
    for (;;) {
        ...
        case 0:                         /* Child */
            ...
            // reset signal handler of parent
